...
do echo "$major $(grep -c "$major" majors.txt)"
done | sort -k2,2nr -k1,1
for name in "${unique_names[@]}"
do if [ $(grep -c "$name" majors.txt) -gt 1 ]; then multiple_major_students="$multiple_major_students $name" 
else single_major_count=$((single_major_count+1)) 
fi
done
echo " "
