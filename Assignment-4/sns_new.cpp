...
vector < priority_queue<action, vector<action>, cmp> feed_queue;
...
void pushUpdate()
{
    while (true)
    {
        ...
        pthread_mutex_lock(&mutex); // to make the pop operation atomic
        if (!monitor.empty())
        {
...
int main()
{
    ...
    pthread_t push_update;
    for (int i = 0; i < 25; i++)
    {
        pthread_create(&push_update, NULL, pushUpdate, NULL);
        pthread_join(push_update, NULL);
    }
